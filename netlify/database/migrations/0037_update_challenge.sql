-- Let the campaign team run the weekly briefs without a developer.
--
-- Issue #67's core requirement. When it was written no challenge existed and
-- the ask was full CRUD; the five stages have since been seeded with real
-- windows, so what the team actually needs is the half that recurs every
-- week: edit the title, write the brief, adjust the points, and flip a stage
-- between draft, active and closed. Briefs drop at the weekend, which is
-- exactly when a solo developer should not be on call.
--
-- The gates this leans on already exist and are already tested: submit_entry
-- refuses any challenge whose status is not active (P0203), the creator page
-- only surfaces active challenges, and closing stops new submissions while
-- leaving existing entries reviewable. This adds only the audited edit.
--
-- A week whose window has ended is read-only. The points and wording of a
-- finished week are part of how its winners were decided, and editing them
-- afterwards edits the record rather than the plan.
...
-- ---------------------------------------------------------------------------
-- submit_entry reads the week's own base points.
--
-- Regenerated from 0025, its latest definition, with one line changed. The
-- lineage guard fails the build if any raise is dropped.
